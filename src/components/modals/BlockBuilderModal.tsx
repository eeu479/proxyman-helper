...
const BlockBuilderModal = ({
  ...
  // Derived values
  const pathParams = [...builderPath.matchAll(/\{([^}]+)\}/g)].map((m) => m[1]);
  ...
  const undefinedTokens = (() => {
    const tokens = [...builderResponseTemplate.matchAll(/\{\{([^}]+)\}\}/g)]
      .map((m) => m[1]?.trim())
    ...
    >
      ...
      >
        <div className="modal__col">
          ...
          <div className="modal__col" style={{ gap: "8px" }}>
            <div className="modal__identity-strip">
              <label className="modal__label" style={{ flex: "0 0 110px" }}>
                Method
                <select
                  className="modal__input"
                  value={builderMethod}
              ...
              >
                ...
                <div className="modal__template-tbody" role="rowgroup">
                  {visibleTemplateValues.map((item, index) => {
                    ...
                    const isOnlyRow = visibleTemplateValues.length === 1;
                    return (
                      <div
                        key={item.id}
            ...
            <div className="modal__section-header">
              ...
              <div className="modal__header-actions">
                <button
                  ...
                  type="button"
                  onClick={() =>
                    onAddResponseHeader("Content-Type", "application/json")
                  }
        ...
        <div className="modal__summary">
          {isEditing ? "Editing endpoint:" : "Creates endpoint:"} {previewLine}
        </div>
