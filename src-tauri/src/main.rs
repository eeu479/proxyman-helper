...
async fn read_blocks_from_path(path: &StdPath, library_id: &str) -> Vec<Block> {
...
async fn run_server(app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    *state.active_profile.lock().await = store.active_profile.clone();
    ...
    let app = Router::new()
        ...
        .route("/api/profiles/:profile_name/requests", post(create_request))
        .route(
    ...
    for lib in &profile.libraries {
        if lib.lib_type == "remote" {
            if let Some(ref folder_path) = lib.folder_path {
                ...
                let blocks = read_blocks_from_path(path, &lib.id).await;
    ...
    for lib in &profile.libraries {
        if lib.lib_type == "remote" {
            if let Some(ref folder_path) = lib.folder_path {
                ...
                if let Err(e) = write_blocks_to_path(path, &blocks).await {
                    return (
                        StatusCode::INTERNAL_SERVER_ERROR,
                        Json(json!({ "error": e })),
                    )
